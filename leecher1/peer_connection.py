...
class PeerConnection(threading.Thread):
    def __init__(self, ip, port, piece_manager, peer_id, info_hash, client, sock=None, is_incoming=False, verbose=False):
        ...
        self.pending_blocks = {}  # {piece_index: [begin_offsets]}
        self.requested_blocks = set()  # {(piece_index, begin)}
    ...
    def recvall(self, length):
        ...
        while len(data) < length:
            more_data = self.socket.recv(length - len(data))
            if not more_data:
    ...
    def parse_handshake(self, data):
        ...
        peer_id = data[1 + pstrlen + 8 + 20:].decode('utf-8', errors='ignore')
    ...
    def send_have(self, piece_index):
        ...
        self.send_message(MESSAGE_HAVE, payload)
        if self.verbose:
            print(f"Sent HAVE {piece_index} to {self.ip}:{self.port}")
        ...
        while self.running:
            try:
                ...
                while True:
                    ...
                    payload = self.buffer[5:4 + msg_length]
    ...
    def handle_message(self, msg_id, payload):
        if msg_id == MESSAGE_CHOKE:
            ...
            self.piece_manager.update_piece_availability_for_piece(piece_index, self.remote_peer_id)
            if self.verbose:
                print(f"Peer {self.ip}:{self.port} has piece {piece_index}.")
            self.update_interest()
        elif msg_id == MESSAGE_BITFIELD:
            self.bitfield = payload
            self.piece_manager.update_piece_availability(self.bitfield, self.remote_peer_id)
            if self.verbose:
    ...
    def update_peer_bitfield(self, piece_index):
        byte_index = piece_index // 8
        bit_index = piece_index % 8
        if len(self.bitfield) <= byte_index:
            self.bitfield += b'\x00' * (byte_index - len(self.bitfield) + 1)
        self.bitfield = bytearray(self.bitfield)
        self.bitfield[byte_index] |= 1 << (7 - bit_index)
        self.bitfield = bytes(self.bitfield)
    ...
    def request_pieces(self):
        ...
        while (not self.piece_manager.is_complete() and
            ...
            # First, try to request blocks from in-progress pieces
            piece_requested = False
            for piece_index in list(self.pending_blocks.keys()):
                ...
                blocks = self.pending_blocks[piece_index]
                while blocks and self.current_requests < MAX_CONCURRENT_REQUESTS:
                    begin = blocks.pop(0)
                    if (piece_index, begin) in self.requested_blocks:
                        continue  # Already requested
                    length = min(block_size, piece_length - begin)
                    payload = struct.pack('!III', piece_index, begin, length)
                    self.send_message(MESSAGE_REQUEST, payload)
                    self.current_requests += 1
                    self.requested_blocks.add((piece_index, begin))
                    piece_requested = True
                    if self.verbose:
                        print(f"Requested block {begin} of piece {piece_index} from {self.ip}:{self.port}")
                    if self.current_requests >= MAX_CONCURRENT_REQUESTS:
                        break
                if not blocks:
                    del self.pending_blocks[piece_index]
                if piece_requested:
                    break  # Break to let other peers request blocks
            if not piece_requested:
                # No in-progress pieces or all blocks requested, start a new piece
                ...
                if not self.has_piece_in_bitfield(self.bitfield, piece_index):
                    ...
                    continue
                piece_length = self.piece_manager.get_piece_length(piece_index)
                blocks = [i for i in range(0, piece_length, block_size)]
                self.pending_blocks[piece_index] = blocks
                if self.verbose:
                    print(f"Started downloading piece {piece_index} from {self.ip}:{self.port}")
                # Loop back to request blocks of the new piece
    ...
    def handle_piece(self, payload):
        ...
        if self.verbose:
            print(
                f"Received block for piece {piece_index} (Begin: {begin}, Length: {len(block)}) from {self.ip}:{self.port}")
        self.piece_manager.add_block(piece_index, begin, block)
        self.current_requests -= 1
        self.requested_blocks.discard((piece_index, begin))
        if self.piece_manager.has_piece(piece_index):
            ...
            self.client.send_have_to_all(piece_index)
            # Remove pending blocks for this piece
            self.pending_blocks.pop(piece_index, None)
    ...
    def handle_request(self, payload):
        ...
        if not self.am_choking and self.has_piece(piece_index):
            ...
            if self.verbose:
                print(f"Cannot send piece {piece_index} to peer {self.ip}:{self.port} because we are choking or don't have the piece.")
